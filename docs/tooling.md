...
- `vitest.config.ts`に`@cloudflare/vitest-plugin`を設定し、Workersの実行環境でテストする。
- テスト開始時に一時ディレクトリでlibSQLサーバーを起動し、終了時に停止・削除する。本番DBや開発用DBは使用しない。
- 入退室、途中参加、8人制限、複数タブ、投票確定、二重抽選、終了・次回、セッションの更新などを検証する。
- ローカルDB用のlibsql-serverもmiseで管理する。
...
Workers・DOのテストには、Cloudflareの公式連携を導入した。
...
- **テスト：Vitest 4を採用。** Cloudflareの公式連携とローカルのlibSQLサーバーを使い、DBの読み書きを含むテストを実行する。ブラウザ上の一連の操作もローカル環境で確認した。

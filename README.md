...
- 接続が終わるまでは、部屋と同じ並びの骨格（上部バー・見出し・メンバー・題材の行・投票バー）を出し、中身が入れ替わるときの位置のずれを抑える。
- 骨格のメンバーは3人分の場所を取る。接続後に実際の人数へ入れ替わる。
- ローディング表示はメンバー欄の右に置き、骨格と同じ場所を使う。
- 「もう一人の参加を待っています」の案内は骨格に入れない。1人のままだと接続時にその分だけ下の内容が下へ動く。

### 題材と抽選

- 星は3段階。★は気軽な話、★★は少し考えて話す話、★★★はじっくり話す話。
- 題材は星ごとに区切り、見出しに星・段階の名前・件数を出す。見出しの下にボーダーを引き、その下に題材を1件1行で並べる。行は3列で、余りは次の行に回る。
- 使用済みの題材も一覧に残し、グレーアウトして選択不可にする。
